...
def split_str_into_newlines(text: str, max_line_count: int = 17):
    words = text.split(" ")
    new_text = ""
    for word in words:
        ...
        for obj in scene["scene"]:
            if "character" in obj:
                ...
                character_name = AnimText(
                    ...
                    font_path="assets/NanumGothicBold.ttf",
                    font_size=12,
                    x=4,
                    y=113,
                )
...
def do_audio(sound_effects: List[Dict]):
    ...
    bip = AudioSegment.from_file(
        "assets/sfx general/sfx-blipmale.wav"
    ) + AudioSegment.silent(duration=50)
    blink = AudioSegment.from_file("assets/sfx general/sfx-blink.wav")
    blink -= 10
    badum = AudioSegment.from_file("assets/sfx general/sfx-fwashing.wav")
    long_bip = bip * 100
    long_bip -= 10
    spf = 1 / fps * 1000
    pheonix_objection = AudioSegment.from_file("assets/Phoenix - objection.mp3")
    edgeworth_objection = AudioSegment.from_file(
        "assets/Edgeworth - (English) objection.mp3"
    )
    default_objection = AudioSegment.from_file("assets/Payne - Objection.mp3")
